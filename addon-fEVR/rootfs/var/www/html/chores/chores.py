#!/usr/bin/python
class chore_wheel:
    def __init__(self,db="./chores.sqlite"):
        import cgi
    ...
    def listChores(self,records,stub='stub/chore.stub',inclDisabled=False):
    ...
    def execute(self):
        ...
        header = self.getStub('stub/choreHeader.stub')
        wheelItems = self.getWheelItems(records)
        count = len(wheelItems)
        canvasItems = self.populateWheelItems(wheelItems,'stub/wheelItem.stub')
        canvas = self.spinnerCanvas(canvasItems,'stub/canvas.stub',count)
        choreList = self.listChores(records,stub='stub/chore.stub',inclDisabled=True)
        choreListHeader = self.getStub('stub/choreListHeader.stub')
        footer = self.getStub('stub/footer.stub')
